...
public class PageIndex implements Handler {
    ...
    public void handle(Context context) throws Exception {
        ...
        int count  = jdbc.countMovies();
        html = html + "<h1> The number of movies in the database: (from JDBCConnection)</h1>" 
                    + count;
        
        ArrayList<String> bestHorComs = jdbc.bestHorrorComedy();
        html = html + "<h1> The best movies in the horror and comedy genre are: (from JDBCConnection)</h1>" +
                      "<ul>";
        for (String bestHorCom : bestHorComs) {
            html = html + "<li>" + bestHorCom + "</li>";
        }
        html = html + "</ul>";

        ArrayList<String> nuremberg = jdbc.nurembergMovies();
        html = html + "<h1> The movies with the Nuremberg included in their names are: (from JDBCConnection)</h1>" +
                      "<ul>";
        for (String nurem : nuremberg) {
            html = html + "<li>" + nurem + "</li>";
        }
        html = html + "</ul>";
...
public ArrayList<String> bestHorrorComedy() {
        // Create the ArrayList to return - this time of Movie objects
        ArrayList<String> bhc = new ArrayList<String>();
        ...
        try {
            ...
            // The Query
            String query = "SELECT MVtitle FROM Movie WHERE UPPER(MVType) = \"HORROR\" OR UPPER(MVType) = \"COMEDY\" AND \"noms\" > 0; ";
            ...
            while (results.next()) {
                 // We can lookup a column of the a single record in the
            // result using the column index
                String bestHorrorComedy = results.getString("mvtitle");
                bhc.add(bestHorrorComedy);
            }
            // Close the statement because we are done with it
            statement.close();
        } catch (SQLException e) {
...
public ArrayList<String> nurembergMovies() {
        // Create the ArrayList to return - this time of Movie objects
        ArrayList<String> nuremberg = new ArrayList<String>();
        ...
        try {
            ...
            // The Query
            String query = "SELECT MVtitle FROM Movie WHERE MVtitle LIKE \"%Nuremberg%\";";
            ...
            while (results.next()) {
                 // We can lookup a column of the a single record in the
            // result using the column index
                String nurembergMovies = results.getString("mvtitle");
                nuremberg.add(nurembergMovies);
            }
            // Close the statement because we are done with it
            statement.close();
        } catch (SQLException e) {
